...
TimeMap.prototype.initTimeline = function(bands) {
     ...
     */
	//Stuart change
    //tm.timeline = Timeline.create(tm.tElement, bands, '' , Timeline.GeochronoUnit);
    tm.timeline = Timeline.create(tm.tElement, bands, '' , Timeline.GeochronoUnit);
...
TimeMap.filters = {
    ...
    hidePastFuture: function(item) {
		...
		*/
        var topband = item.timeline.getBand(0),
            maxVisibleDate = topband.getMaxVisibleDate(),
            minVisibleDate = topband.getMinVisibleDate(),
            itemStart = item.getStartTime(),
            itemEnd = item.getEndTime();

        if (itemStart !== undefined) {
            // hide items in the future
            return itemStart > maxVisibleDate._n &&
                // hide items in the past
                (itemEnd < minVisibleDate._n || itemStart < minVisibleDate._n);
        }
        return true;
    },
    ...
    hideFuture: function(item) {
        var maxVisibleDate = item.timeline.getBand(0).getMaxVisibleDate(),
            itemStart = item.getStartTime();
        if (itemStart !== undefined) {
            // hide items in the future
    ...
    showMomentOnly: function(item) {
        var topband = item.timeline.getBand(0),
            momentDate = topband.getCenterVisibleDate(),
            itemStart = item.getStartTime(),
            itemEnd = item.getEndTime();
        if (itemStart !== undefined) {
            // hide items in the future
...
TimeMapDataset = function(timemap, options) {
     ...
     */
	//Stuart
    ds.eventSource = options.eventSource || new Timeline.DefaultEventSource();
 	//ds.eventSource = new Timeline.DefaultEventSource(new SimileAjax.EventIndex(Timeline.GeochronoUnit)); 
...
TimeMapDataset.hybridParser = function(s) {
    ...
    if (isNaN(d)) {
        if (typeof(s) == "string") {
            ...
            if (s.match(/^-?\d{1,7} ?(a\.?d\.?|b\.?c\.?e?\.?|c\.?e\.?)?$/i)) {
...
TimeMapTheme.create = function(theme, options) {
    ...
    if (theme) theme = TimeMap.util.lookup(theme, TimeMap.themes);
	if (!theme) return new TimeMapTheme(options);
